...
async fn process_chapter(
    ...
    let mut rendered = String::with_capacity(stylesheet_header.len() + raw_content.len());
    rendered.push_str(&stylesheet_header);
    if let Err(why) = cmark(parser.into_iter(), &mut rendered) {
        eprintln!("A `pulldown_cmark_to_cmark` parser failed: {why}.");
        return raw_content;
    }
